...
class uploadFile {
     ...
     */
    public function __construct($path = '',$file_name = 'date',$allow_types = '') {
        $this->file_name   = $file_name;//重命名方式代表以时间命名，其他则使用给予的名称
        $this->save_path   = $this->save_path.$path;
        $this->allow_types = $allow_types == '' ? 'jpg|gif|png|zip|tar|gz|log|txt|jpeg|' : $allow_types;
    }
    ...
    public function upload_file($files) {
        ...
        if($error == 0 && is_uploaded_file($tmp_name)) {
            ...
            $new_name = $this->file_name != 'date' ? $this->file_name.'.'.$this->ext : date('YmdHis').rand(0, 9999).'.'.$this->ext;//设置新文件名
